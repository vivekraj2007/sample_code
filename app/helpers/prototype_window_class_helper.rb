...
module PrototypeWindowClassHelper
    ...
    def link_to_prototype_window( name, window_id, options = {} , html_options = {} )
        ...
        options.merge!( :windowParameters => {} ) if !options.has_key?(:windowParameters)
        js_code = "#{before};" if before
        js_code ="#{js_code} var win = new Window( '#{window_id}', #{params_for_javascript(options) } );  win.show();  win.setDestroyOnClose();"
        js_code = "#{js_code} #{after};"
        content_tag(
               "a", name, 
               html_options.merge({ 
                 :href => html_options[:href] || "#", 
